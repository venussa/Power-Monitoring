...
<template>
    <div style="background: #0a8de2; height: 100%">
      ...
      <div class="col-md-12" style="text-align: center; color: #fff; margin-top: 30px;position: relative; z-index: 999">
          <div><div style="display: inline-block"><i class="home-icon" /></div></div>
          <p style="font-size: 27px; font-weight: 600;margin-top: 30px;">Power Monitoring</p>
          <p style="font-size: 19px;">See your building's Power Consumption!</p>
          <p>Application made by Peaple Hearts</p>
          <b-link v-if="token === null" v-b-modal.modal-primary  class="btn btn-primary btn-white">Log In</b-link>
          <b-link v-if="token !== null" :to="{ name: 'node' }" class="btn btn-primary btn-white">View Nodes</b-link>
          <p v-if="token !== null" style="padding: 10px; font-size: 10px">
  ...
  methods: {
    ...
    login() {
      ...
      http.finally(() => {
        ...
        sessionStorage.setItem('name', api.data.name)
        sessionStorage.setItem('avatar', api.data.avatar)
